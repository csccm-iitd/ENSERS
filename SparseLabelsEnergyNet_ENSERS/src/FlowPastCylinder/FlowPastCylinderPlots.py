...
class Plots:
    ...
    def plotPred(self, plotData: Dict, plotParams, savePath: str):
        ...
        for i in range(numPlots):
            for j in range(ch):
                t_ij = target[i, idxLs[i], j].reshape(self.imDim)[2:-2, 2:-2]
                p_ij = pred[i, idxLs[i], j].reshape(self.imDim)[2:-2, 2:-2]
                c_max = np.max(t_ij)#np.max(np.array([t_ij, p_ij]))
                c_min = np.min(t_ij)#np.min(np.array([t_ij, p_ij]))
                ...
                error_ij = t_ij- p_ij
                c_max_error = np.max(np.abs(error_ij))
                c_min_error = np.min(np.abs(error_ij))
    ...
    def violinplot(self, l2Error, plotParams, savePath):
        ...
        for m in range(l2Error.shape[0]):
            ...
            for body in vp['bodies']:
                ...
                body.set_alpha(0.4)
        ax.set(xlim=(0, 130), xticks=pp.xticks, xlabel=pp.xlabel, xticklabels=pp.xticklabels,
