...
class DatasetClass(Dataset):
    ...
    def getTrainData(self):
        ...
        rawDataNodesLoc = self.Loc(rawDataLen, self.hp.numDataNodes)  # (rawDataLen, 3, numDataNodes)
        self.dataNodesLocTrain = self.catTimeStep(rawDataNodesLoc, dataIdxTrainLs)  # (numSampTrain, timeStepModel, 3, numDataNodes) 

        rawDataNodesValue = self.Value(rawDataNodesLoc)  # (rawDataLen, 3, numDataNodes)              
        self.stateDataTrain = self.catTimeStep(rawDataNodesValue, dataIdxTrainLs)  # (numSampTrain, timeStepModel, 3, numDataNodes)

        # ---------------------- sensor Data nodes & values ------------------------
        rawSensorLocTrain = self.Loc(rawDataLen, self.hp.numSensorTrain)  # (rawDataLen, 3, numSensorTrain)
        self.sensorLocTrain = self.catTimeStep(rawSensorLocTrain, dataIdxTrainLs)  # (numSampTrain, timeStepModel, 3, numSensorTrain)

        rawSensorValueTrain = self.Value(rawSensorLocTrain)  # (rawDataLen, 3, numSensorTrain)        
        self.sensorDataTrain = self.catTimeStep(rawSensorValueTrain, dataIdxTrainLs)  # (numSampTrain, timeStepModel, 3, numSensorTrain)
    ...
    def getTestData(self):
        ...
        # ---------------------- state Data nodes & values -------------------------
        self.stateDataTest = self.catTimeStep(self.rawData.data, dataIdxTestLs)  # (numSampTest, timeStepModel, 3, numNodes)

        # ---------------------- sensor Data nodes & values ------------------------
        rawSensorLocTest = self.Loc(rawDataLen, self.hp.numSensorTest)  # (rawDataLen, 3, numSensorTest)   
        self.sensorLocTest = self.catTimeStep(rawSensorLocTest, dataIdxTestLs)  # (numSampTest, timeStepModel, 3, numSensorTest)

        rawSensorValueTest = self.Value(rawSensorLocTest, noise=self.hp.noise)  # (rawDataLen, 3, numSensorTest)            
        self.sensorDataTest = self.catTimeStep(rawSensorValueTest, dataIdxTestLs)  # (numSampTest, timeStepModel, 3, numSensorTest)
    ...
    def Loc(self, numSamp, numDataLoc):
        ...
        return T.cat([dataLoc[:, None]]*3, dim=1)
    ...
    def catTimeStep(self, rawNodes, idxLs):
        ...
        Args:
            rawNodes (Tensor): (rawDataLen, 3, numDataNodes)
            idxLs (Tensor): (numSamp)
        Returns:
            catValue (Tensor): (numSamp, timeStepModel, 3, numDataNodes)
        """
        catValue = []
        for i in range(self.hp.timeStepModel):
            nodesValue = rawNodes[idxLs + i]  # (numSamp, 3, numDataNodes)
            catValue.append(nodesValue[:, None])  # (numSamp, 1, 3, numDataNodes)

        return T.cat(catValue, dim=1)  # (numSamp, timeStepModel, 3, numDataNodes)
    ...
    def Value(self, NodesLoc, noise=None):
        ...
        Args:
            NodesLoc (Tensor): (rawDataLen, 3, numDataNodes).
            noise (float, optional): noise level in SNRdb (default: `None`).
        Vars:
            self.rawData.data (Tensor): (timeStep, 3, numNodes)
        Returns:
            Value (Tensor): (rawDataLen, 3, numDataNodes)
        """
